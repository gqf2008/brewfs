...
**状态：opt-in 且实验性。** 真实 Explorer/Finder 回收站集成**不可用**（实测证实：Windows 资源管理器删除在 WinFsp 挂载上不进入回收站；macOS 26 系统阻止 macFUSE 内核扩展、FUSE-T 以 NFS 卷挂载导致 Finder 废纸篓不可用）。虚拟视图本身可用（浏览/恢复/清空），但因系统回收站入口对用户不可见，**默认关闭**，仅显式开启者使用。
