//! Prometheus-style text rendering for `FsStats`.
//!
//! Split out of `stats.rs` (part of the writer/stats decomposition): the
//! `.stats` virtual-file renderer is a pure formatting function over the live
//! counters. No behavior changes.

use crate::vfs::stats::{FsStats, ORD};

pub(crate) fn render(stats: &FsStats) -> String {
    ...
    out.push_str(&format!(
        ...
        stats.fuse_read_ops.load(ORD)
    ));
    out.push_str(&format!(
        "brewfs_fuse_read_bytes_total {}\n",
        stats.fuse_read_bytes.load(ORD)
    ));
    out.push_str(&format!(
        "brewfs_fuse_read_lat_us_total {}\n",
        stats.fuse_read_lat_us.load(ORD)
    ));
    out.push_str(&format!(
        "brewfs_fuse_write_ops_total {}\n",
        stats.fuse_write_ops.load(ORD)
    ));
    out.push_str(&format!(
        "brewfs_fuse_write_bytes_total {}\n",
        stats.fuse_write_bytes.load(ORD)
    ));
    out.push_str(&format!(
        "brewfs_fuse_write_lat_us_total {}\n",
        stats.fuse_write_lat_us.load(ORD)
    ));
    out.push_str(&format!(
        "brewfs_fuse_lookup_ops_total {}\n",
        stats.fuse_lookup_ops.load(ORD)
    ));
    out.push_str(&format!(
        "brewfs_fuse_lookup_lat_us_total {}\n",
        stats.fuse_lookup_lat_us.load(ORD)
    ));
    out.push_str(&format!(
        "brewfs_fuse_getattr_ops_total {}\n",
        stats.fuse_getattr_ops.load(ORD)
    ));
    out.push_str(&format!(
        "brewfs_fuse_getattr_lat_us_total {}\n",
        stats.fuse_getattr_lat_us.load(ORD)
    ));
    out.push_str(&format!(
        "brewfs_fuse_open_ops_total {}\n",
        stats.fuse_open_ops.load(ORD)
    ));
    out.push_str(&format!(
        "brewfs_fuse_create_ops_total {}\n",
        stats.fuse_create_ops.load(ORD)
    ));
    out.push_str(&format!(
        "brewfs_fuse_unlink_ops_total {}\n",
        stats.fuse_unlink_ops.load(ORD)
    ));
    out.push_str(&format!(
        "brewfs_fuse_readdir_ops_total {}\n",
        stats.fuse_readdir_ops.load(ORD)
    ));
    out.push_str(&format!(
        "brewfs_fuse_flush_ops_total {}\n",
        stats.fuse_flush_ops.load(ORD)
    ));
    out.push_str(&format!(
        "brewfs_fuse_flush_lat_us_total {}\n",
        stats.fuse_flush_lat_us.load(ORD)
    ));
    out.push_str(&format!(
    ...
    out.push_str(&format!(
        ...
        stats.meta_ops.load(ORD)
    ));
    out.push_str(&format!(
        "brewfs_meta_lat_us_total {}\n",
        stats.meta_lat_us.load(ORD)
    ));
    out.push_str(&format!(
        "brewfs_meta_txn_ops_total {}\n",
        stats.meta_txn_ops.load(ORD)
    ));
    out.push_str(&format!(
        "brewfs_meta_txn_lat_us_total {}\n",
        stats.meta_txn_lat_us.load(ORD)
    ));

    // VFS diagnostic timing
    out.push_str(&format!(
        "brewfs_vfs_create_total_ops_total {}\n",
        stats.vfs_create_total_ops.load(ORD)
    ));
    out.push_str(&format!(
        "brewfs_vfs_create_total_lat_us_total {}\n",
        stats.vfs_create_total_lat_us.load(ORD)
    ));
    out.push_str(&format!(
        "brewfs_vfs_create_meta_ops_total {}\n",
        stats.vfs_create_meta_ops.load(ORD)
    ));
    out.push_str(&format!(
        "brewfs_vfs_create_meta_lat_us_total {}\n",
        stats.vfs_create_meta_lat_us.load(ORD)
    ));
    out.push_str(&format!(
        "brewfs_vfs_unlink_total_ops_total {}\n",
        stats.vfs_unlink_total_ops.load(ORD)
    ));
    out.push_str(&format!(
        "brewfs_vfs_unlink_total_lat_us_total {}\n",
        stats.vfs_unlink_total_lat_us.load(ORD)
    ));
    out.push_str(&format!(
        "brewfs_vfs_unlink_lookup_ops_total {}\n",
        stats.vfs_unlink_lookup_ops.load(ORD)
    ));
    out.push_str(&format!(
        "brewfs_vfs_unlink_lookup_lat_us_total {}\n",
        stats.vfs_unlink_lookup_lat_us.load(ORD)
    ));
    out.push_str(&format!(
        "brewfs_vfs_unlink_stat_ops_total {}\n",
        stats.vfs_unlink_stat_ops.load(ORD)
    ));
    out.push_str(&format!(
        "brewfs_vfs_unlink_stat_lat_us_total {}\n",
        stats.vfs_unlink_stat_lat_us.load(ORD)
    ));
    out.push_str(&format!(
        "brewfs_vfs_unlink_meta_ops_total {}\n",
        stats.vfs_unlink_meta_ops.load(ORD)
    ));
    out.push_str(&format!(
        "brewfs_vfs_unlink_meta_lat_us_total {}\n",
        stats.vfs_unlink_meta_lat_us.load(ORD)
    ));
    out.push_str(&format!(
        "brewfs_vfs_unlink_recent_ops_total {}\n",
        stats.vfs_unlink_recent_ops.load(ORD)
    ));
    out.push_str(&format!(
        "brewfs_vfs_unlink_recent_lat_us_total {}\n",
        stats.vfs_unlink_recent_lat_us.load(ORD)
    ));
    out.push_str(&format!(
        "brewfs_vfs_setattr_recent_remove_ops_total {}\n",
        stats.vfs_setattr_recent_remove_ops.load(ORD)
    ));
    out.push_str(&format!(
        "brewfs_vfs_setattr_recent_remove_lat_us_total {}\n",
        stats.vfs_setattr_recent_remove_lat_us.load(ORD)
    ));
    out.push_str(&format!(
        "brewfs_vfs_setattr_recent_get_mut_ops_total {}\n",
        stats.vfs_setattr_recent_get_mut_ops.load(ORD)
    ));
    out.push_str(&format!(
        "brewfs_vfs_setattr_recent_get_mut_lat_us_total {}\n",
        stats.vfs_setattr_recent_get_mut_lat_us.load(ORD)
    ));
    out.push_str(&format!(
        "brewfs_vfs_read_dirty_probe_ops_total {}\n",
        stats.vfs_read_dirty_probe_ops.load(ORD)
    ));
    out.push_str(&format!(
        "brewfs_vfs_read_dirty_probe_lat_us_total {}\n",
        stats.vfs_read_dirty_probe_lat_us.load(ORD)
    ));
    out.push_str(&format!(
        "brewfs_vfs_read_handle_ops_total {}\n",
        stats.vfs_read_handle_ops.load(ORD)
    ));
    out.push_str(&format!(
        "brewfs_vfs_read_handle_lat_us_total {}\n",
        stats.vfs_read_handle_lat_us.load(ORD)
    ));
    out.push_str(&format!(
        "brewfs_vfs_read_overlay_ops_total {}\n",
        stats.vfs_read_overlay_ops.load(ORD)
    ));
    out.push_str(&format!(
        "brewfs_vfs_read_overlay_lat_us_total {}\n",
        stats.vfs_read_overlay_lat_us.load(ORD)
    ));

    // Object storage
    out.push_str(&format!(
        "brewfs_s3_get_ops_total {}\n",
        stats.s3_get_ops.load(ORD)
    ));
    out.push_str(&format!(
        "brewfs_s3_get_bytes_total {}\n",
        stats.s3_get_bytes.load(ORD)
    ));
    out.push_str(&format!(
        "brewfs_s3_get_lat_us_total {}\n",
        stats.s3_get_lat_us.load(ORD)
    ));
    out.push_str(&format!(
        "brewfs_s3_put_ops_total {}\n",
        stats.s3_put_ops.load(ORD)
    ));
    out.push_str(&format!(
        "brewfs_s3_put_bytes_total {}\n",
        stats.s3_put_bytes.load(ORD)
    ));
    out.push_str(&format!(
        "brewfs_s3_put_lat_us_total {}\n",
        stats.s3_put_lat_us.load(ORD)
    ));
    out.push_str(&format!(
        "brewfs_s3_put_prepare_lat_us_total {}\n",
        stats.s3_put_prepare_lat_us.load(ORD)
    ));
    out.push_str(&format!(
        "brewfs_s3_put_cache_lat_us_total {}\n",
        stats.s3_put_cache_lat_us.load(ORD)
    ));
    out.push_str(&format!(
        "brewfs_s3_del_ops_total {}\n",
        stats.s3_del_ops.load(ORD)
    ));
    out.push_str(&format!(
    ...
    out.push_str(&format!(
        ...
        stats.buf_dirty_bytes.load(ORD)
    ));
    out.push_str(&format!(
        "brewfs_buffer_read_bytes {}\n",
        stats.buf_read_bytes.load(ORD)
    ));
    out.push_str(&format!(
    ...
    out.push_str(&format!(
        ...
        stats.cache_hits.load(ORD)
    ));
    out.push_str(&format!(
        "brewfs_cache_misses_total {}\n",
        stats.cache_misses.load(ORD)
    ));
    out.push_str(&format!(
