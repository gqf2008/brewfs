...
const BREWFS_BIN_IN_VM: &str = "/usr/local/bin/brewfs";
const BREWFS_MOUNTPOINT: &str = "/mnt/brewfs";
const BREWFS_DATA_DIR: &str = "/tmp/brewfs-data";
const BREWFS_LOG_PATH: &str = "/var/log/brewfs.log";
const BREWFS_META_DIR: &str = "/tmp/brewfs-meta";
...
async fn dump_debug_info(vm: &mut Machine) -> Result<String> {
    ...
    s.push_str("\n=== logs (brewfs) ===\n");
    s.push_str(&get_file_tail(vm, BREWFS_LOG_PATH, 300).await?);

    s.push_str("\n=== sqlite dir ===\n");
    s.push_str(&exec_check(vm, "ls -la /tmp/brewfs-db 2>/dev/null || true").await?);
    s.push_str(&exec_check(vm, "stat /tmp/brewfs-db/metadata.db 2>/dev/null || true").await?);
...
fn get_brewfs_binary_path() -> Result<PathBuf> {
    if let Ok(path) = std::env::var("CARGO_BIN_EXE_brewfs") {
    ...
    let debug_path = target_dir.join("debug/brewfs");
    if debug_path.exists() {
        return Ok(debug_path);
    }

    let release_path = target_dir.join("release/brewfs");
    if release_path.exists() {
        return Ok(release_path);
    }

    anyhow::bail!(
        "brewfs binary not found at {:?} or {:?}. Build it first, e.g. `cargo build -p brewfs`.",
        debug_path,
        release_path
    );
}
...
async fn wait_for_mount(vm: &mut Machine, timeout: Duration) -> Result<()> {
    ...
    loop {
        ...
        if start.elapsed() > timeout {
            ...
            let log = get_file_tail(vm, BREWFS_LOG_PATH, 800)
                .await
                .unwrap_or_else(|e| e.to_string());
            anyhow::bail!(
                "timeout waiting for brewfs mount at {}\n\n=== ps (brewfs) ===\n{}\n=== logs (brewfs) ===\n{}",
                BREWFS_MOUNTPOINT,
                ps,
                log
            );
...
async fn run_brewfs_mount_root(vm: &mut Machine, meta_url: &str) -> Result<()> {
    exec_check(
        vm,
        &format!("rm -f {log} && touch {log}", log = BREWFS_LOG_PATH),
    )
    .await?;

    exec_check(
        vm,
        &format!("rm -rf {d} && mkdir -p {d}", d = BREWFS_META_DIR),
    )
    .await?;
    exec_check(vm, &format!("chmod 1777 {d}", d = BREWFS_META_DIR)).await?;
...
async fn basic_fs_checks(vm: &mut Machine) -> Result<()> {
    exec_check(
        vm,
        &format!("sh -lc 'echo hello > {}/a'", BREWFS_MOUNTPOINT),
    )
    .await?;
    exec_check(
        vm,
        &format!("sh -lc 'cat {}/a | grep -q hello'", BREWFS_MOUNTPOINT),
    )
    .await?;
    exec_check(
        vm,
        &format!("mv {}/a {}/b", BREWFS_MOUNTPOINT, BREWFS_MOUNTPOINT),
    )
    .await?;
    exec_check(vm, &format!("rm {}/b", BREWFS_MOUNTPOINT)).await?;

    exec_check(vm, &format!("mkdir {}/dir", BREWFS_MOUNTPOINT)).await?;
    exec_check(vm, &format!("rmdir {}/dir", BREWFS_MOUNTPOINT)).await?;
...
async fn run_posix_edge_cases(vm: &mut Machine) -> Result<()> {
    exec_check(
        ...
        &format!(
            ...
            BREWFS_MOUNTPOINT, BREWFS_MOUNTPOINT
        ),
    )
    .await?;
    ...
    exec_check(
        vm,
        &format!("test -f {}/edge/dir2/moved", BREWFS_MOUNTPOINT),
    )
    .await?;
    exec_check(
        vm,
        &format!("test ! -f {}/edge/dir1/file", BREWFS_MOUNTPOINT),
    )
    .await?;
...
async fn run_concurrency_smoke(vm: &mut Machine) -> Result<()> {
    exec_check(vm, &format!("mkdir -p {}/concurrency", BREWFS_MOUNTPOINT)).await?;
...
for i in $(seq 1 4); do \
  for j in $(seq 1 50); do echo \"$i-$j\" >> {}/concurrency/file-$i; done; \
    ...
    let total1 = exec_check(
        vm,
        &format!("sh -lc 'wc -l {}/concurrency/file-*'", BREWFS_MOUNTPOINT),
    )
    .await?;
...
async fn kill_brewfs_best_effort(vm: &mut Machine) -> Result<()> {
    let _ = vm.exec("pkill -TERM brewfs >/dev/null 2>&1 || true; sleep 1; pkill -KILL brewfs >/dev/null 2>&1 || true").await;
    Ok(())
}

async fn run_in_vm(vm: &mut Machine, brewfs_bin: &Path) -> Result<()> {
    let r = run_full(vm, brewfs_bin).await;
...
async fn start_vm_and_run(brewfs_bin: PathBuf) -> Result<()> {
    tracing_subscriber_init();

    tracing::info!("using brewfs binary at {:?}", brewfs_bin);
    ...
    let brewfs_bin = std::sync::Arc::new(brewfs_bin);
    with_machine(&image, &config, move |vm| {
        let brewfs_bin = std::sync::Arc::clone(&brewfs_bin);
        Box::pin(async move { run_in_vm(vm, brewfs_bin.as_ref()).await })
    })
    .await?;
...
async fn run_full(vm: &mut Machine, brewfs_bin: &Path) -> Result<()> {
    install_deps(vm).await?;
    setup_unprivileged_user(vm, "tester").await?;
    upload_brewfs(vm, brewfs_bin).await?;

    let db_dir = "/tmp/brewfs-db";
    let db_path = "/tmp/brewfs-db/metadata.db";
    ...
    run_brewfs_mount_root(vm, &meta_url).await?;
    ...
    kill_brewfs_best_effort(vm).await?;
    ...
    .await?;
    exec_check(vm, &format!("test -d {}/pdir/sub", BREWFS_MOUNTPOINT)).await?;
...
async fn upload_brewfs(vm: &mut Machine, brewfs_bin: &Path) -> Result<()> {
    vm.upload(brewfs_bin, Path::new("/usr/local/bin")).await?;
    exec_check(vm, &format!("chmod +x {}", BREWFS_BIN_IN_VM)).await?;
    Ok(())
}

#[tokio::test]
#[ignore]
async fn test_brewfs_kvm() -> Result<()> {
    let brewfs_bin = get_brewfs_binary_path()?;
    start_vm_and_run(brewfs_bin).await
}
