...
const CLIENT_FUSE_MOUNT: &str = "/mnt/brewfs";

const HOST_DIAG_ROOT: &str = "/tmp/brewfs-qlean-multinode";
const DIST_TESTS_DIR: &str = "/opt/brewfs-dist-tests/distributed-tests";
const DIST_RESULTS_DIR: &str = "/tmp/brewfs-results";
const DIST_WORKDIR: &str = "/root/brewfs-dist";
...
async fn maybe_run_single_vm_probe(image: &qlean::Image, config: &MachineConfig) -> Result<()> {
    let enabled = std::env::var("BREWFS_QLEAN_SINGLE_VM_CHECK")
        .map(|v| v == "1")
        .unwrap_or(false);
    if !enabled {
        return Ok(());
...
async fn stage_distributed_tests(vm: &mut Machine, repo_root: &Path) -> Result<()> {
    let dist_tests = repo_root.join("tests/scripts/distributed-tests");
    ...
    if !dist_bin.exists() {
        anyhow::bail!(
            "brewfs example binary not found at {} (build it with `cargo build -p brewfs --example persistence_demo --release`)",
            dist_bin.display()
        );
    }

    exec_check(vm, "mkdir -p /opt/brewfs-dist-tests").await?;

    vm.upload(&dist_tests, Path::new("/opt/brewfs-dist-tests"))
        .await
    ...
    vm.upload(&dist_bin, Path::new(&format!("{DIST_WORKDIR}/bin")))
        .await
        .context("upload brewfs persistence_demo binary")?;
...
fn render_cluster_env(
    ...
    let tests_root = repo_root.join("tests/scripts/distributed-tests");
...
BREWFS_BUILD=0
BREWFS_EXAMPLE="persistence_demo"
BREWFS_BIN_LOCAL="{bin_local}"
BREWFS_META_BACKEND="{backend}"
BREWFS_MOUNT_DIR="/mnt/brewfs"
BREWFS_META_DIR="/var/lib/brewfs/meta"
BREWFS_DATA_DIR="/var/lib/brewfs/data"
BREWFS_LOG_DIR="/var/log/brewfs"
...
async fn run_distributed_tests(
  ...
  echo "=== brewfs log ($node) ===";
  ssh -o BatchMode=yes -o StrictHostKeyChecking=no -i /root/.ssh/id_ed25519 root@$node \
    "tail -n 200 /var/log/brewfs/brewfs-$node.log 2>/dev/null || true" || true;
done"#,
    ...
    let host_results_root = repo_root.join("tests/scripts/distributed-tests/results");
    coordinator
...
async fn cleanup_postgres_metadata_remote(vm: &mut Machine, host: &str) -> Result<()> {
    let cmd = format!(
        "sh -lc 'PGPASSWORD=brewfs psql -h {host} -p 15432 -U brewfs -d database -c \"DROP SCHEMA public CASCADE; CREATE SCHEMA public;\"'",
    );
    exec_check(vm, &cmd).await?;
    Ok(())
}
...
fn backend_urls_for(backend: MetaBackend, default_host: &str) -> Vec<String> {
    match backend {
        ...
        MetaBackend::Postgres => vec![format!(
            "postgres://brewfs:brewfs@{default_host}:15432/database"
        )],
    }
}
...
#[tokio::test]
#[ignore]
async fn test_brewfs_qlean_multinode_smoke() -> Result<()> {
