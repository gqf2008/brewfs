    ...
    TRACING_INIT.call_once(|| {
        ...
        if let Some(path) = chrome_trace {
            ...
            CHROME_GUARD
                ...
                .replace(guard);
            eprintln!("[brewfs_bench] Chrome trace enabled: {}", path.display());
...
struct BenchArgs {
    ...
    block_mb: usize,
    #[arg(long, env = "BREWFS_BENCH_BIG_FILE_MB", default_value_t = 512)]
    big_file_mb: usize,
    #[arg(long, env = "BREWFS_BENCH_SMALL_FILE_KB", default_value_t = 128)]
    small_file_kb: usize,
    #[arg(long, env = "BREWFS_BENCH_SMALL_FILE_COUNT", default_value_t = 100)]
    small_file_count: usize,
    #[arg(long, env = "BREWFS_BENCH_THREADS", default_value_t = 4)]
    threads: usize,
    #[arg(long, env = "BREWFS_BENCH_SAMPLE_SIZE", default_value_t = 10)]
    sample_size: usize,
    ...
    #[arg(long, env = "BREWFS_BENCH_META_TIKV_PD_ENDPOINTS")]
    meta_tikv_pd_endpoints: Option<String>,
    #[arg(
        long,
        env = "BREWFS_BENCH_META_TIKV_NAMESPACE",
        default_value = "brewfs"
    )]
    meta_tikv_namespace: String,
    #[arg(long, env = "BREWFS_BENCH_DATA_DIR")]
    data_dir: Option<PathBuf>,
    #[arg(long, env = "BREWFS_BENCH_BACKEND", value_enum, default_value_t = BackendKind::Local)]
    backend: BackendKind,
    #[arg(long, env = "BREWFS_BENCH_S3_BUCKET")]
    s3_bucket: Option<String>,
    #[arg(long, env = "BREWFS_BENCH_S3_REGION")]
    s3_region: Option<String>,
    #[arg(long, env = "BREWFS_BENCH_S3_ENDPOINT")]
    s3_endpoint: Option<String>,
    #[arg(long, env = "BREWFS_BENCH_S3_FORCE_PATH_STYLE")]
    s3_force_path_style: Option<String>,
    #[arg(long, env = "BREWFS_BENCH_FLAMEGRAPH")]
    flamegraph: Option<String>,
    #[arg(long, env = "BREWFS_BENCH_CHROME")]
...
impl BenchConfig {
    fn from_env() -> Self {
        let args = BenchArgs::parse_from(["brewfs-bench"]);
        let chrome_trace = match args.chrome.as_deref() {
            Some(value) if parse_env_bool(Some(value)) => {
                Some(PathBuf::from("brewfs_bench_trace.json"))
            }
        ...
        let backend = match args.backend {
            ...
            BackendKind::S3 => {
                let bucket = args.s3_bucket.unwrap_or_else(|| {
                    panic!("BREWFS_BENCH_S3_BUCKET must be set when backend is s3")
                });
        ...
        let meta_backend = match args.meta_backend {
            ...
            MetaBackendKind::Etcd => {
                let urls = args
                    ...
                    .unwrap_or_default();
                if urls.is_empty() {
                    panic!("BREWFS_BENCH_META_ETCD_URLS must be set when meta backend is etcd");
                }
            ...
            MetaBackendKind::TiKv => {
                ...
                if pd_endpoints.is_empty() {
                    panic!(
                        "BREWFS_BENCH_META_TIKV_PD_ENDPOINTS must be set when meta backend is tikv"
                    );
                }
                MetaBackend::TiKv {
                    pd_endpoints,
...
fn create_root_dir(cfg: &BenchConfig) -> Result<BenchRoot> {
    if let Some(dir) = cfg.data_dir.as_ref() {
        ...
        let stamp = SystemTime::now()
            ...
            .unwrap_or_default()
            .as_millis();
        let run_dir = base.join(format!("brewfs_bench_{}", stamp));
...
fn create_baseline_root(cfg: &BenchConfig) -> Result<BenchRoot> {
    if let Some(dir) = cfg.data_dir.as_ref() {
        ...
        let stamp = SystemTime::now()
            ...
            .unwrap_or_default()
            .as_millis();
        let run_dir = base.join(format!("brewfs_baseline_{}", stamp));
...
fn bench_big_files(c: &mut Criterion) {
    ...
    let mut group = c.benchmark_group("brewfs_big_file");
...
fn bench_small_files(c: &mut Criterion) {
    let cfg = BenchConfig::from_env();
    let runtime = tokio_runtime(cfg.threads);
    let mut group = c.benchmark_group("brewfs_small_file");
    group.sample_size(cfg.sample_size);
...
fn bench_small_stats(c: &mut Criterion) {
    let cfg = BenchConfig::from_env();
    let runtime = tokio_runtime(cfg.threads);
    let mut group = c.benchmark_group("brewfs_stat");
    group.sample_size(cfg.sample_size);
    group.throughput(Throughput::Elements(cfg.small_total_files()));
...
fn build_criterion() -> Criterion {
    ...
    if cfg.flamegraph {
        eprintln!("[brewfs_bench] Flamegraph profiler enabled");
